...
div.stButton > button { width:100%; text-align:left; background:transparent; border:none;
  color:#aaa; padding:10px 14px; border-radius:8px; font-size:14px; margin-bottom:4px; }
...
def find_alternatives(medicine_name):
    ...
    ].sort_values('price(Rs)')
    alt_list = []
    for _, row in alternatives.iterrows():
        saving = round(original['price(Rs)'] - row['price(Rs)'], 2)
        saving_pct = round((saving / original['price(Rs)']) * 100, 1)
        alt_list.append({'name': row['name'], 'price': round(row['price(Rs)'], 2),
            'saving': saving, 'saving_pct': saving_pct,
            'manufacturer': row['manufacturer_name'], 'pack_size': row['pack_size_label']})
    return {'original_name': original['name'], 'original_price': round(original['price(Rs)'], 2),
            'original_composition': original['composition'],
            'original_manufacturer': original['manufacturer_name'],
            'original_pack': original['pack_size_label'], 'alternatives': alt_list}
...
def detect_overcharge(medicine_name, billed_price):
    ...
    return {'medicine': result['original_name'], 'billed_price': billed_price,
            'actual_price': db_price, 'overcharge': overcharge,
            'overcharge_pct': overcharge_pct, 'is_overcharged': overcharge > 0,
            'composition': result['original_composition'], 'alternatives': result['alternatives']}
...
def nlp_extract_medicines(text, key):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {'role': 'user', 'content': 'Extract medicines from:\n\n' + text}
...
if page == 'search':
    ...
    with tab1:
        ...
        uploaded = st.file_uploader('Choose file', type=['jpg','jpeg','png','pdf'], help='Supports JPG, PNG images and PDF files')
        if uploaded is not None:
            ...
            if st.button('🚀 Analyze Prescription', type='primary', use_container_width=True):
                ...
                if all_text.strip():
                    ...
                    if medicines_found:
                        ...
                        st.markdown('---')
                        st.markdown('### 💊 Step 3 — Alternatives')
                        total_branded = 0
                        total_generic = 0
                        summary_rows = []
            ...
            st.markdown('<div style="background:#1e2130;border:2px dashed #2d3250;border-radius:12px;padding:40px;text-align:center;color:#888;margin-top:20px;"><div style="font-size:3rem;">📋</div><div style="font-size:1.1rem;margin-top:10px;">Drag and drop your prescription here</div><div style="font-size:0.85rem;margin-top:6px;">Supports JPG, PNG, PDF</div></div>', unsafe_allow_html=True)
    with tab2:
